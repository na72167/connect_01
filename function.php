...
//==デバックやDB接続関係の設定を纏めた物== get

//ログ出力関係の設定(本番環境に移す際は切っておく。)
ini_set('log_errors','on');
//ログの出力ファイルを指定
ini_set('error_log','php.log');

//デバッグ用関数(途中までの処理が出来ているか確認する用)
...
function appendGetParam($arr_del_key = array()){
  ...
  if(!empty($_GET)){
    ...
    //false
    //mb_substrについてのメモ
    //第一引数に代入先の変数,第二が取得したい要素の開始位置,第三が終了位置を示す。第三の"UTF-8"は取得先が文字列の場合は設定する。
    //こっちは$_GET内に何も無い為場合の処理の為、そもそもクエリパラメータを受け渡さない様にする為第二引数に-1を設定している。
